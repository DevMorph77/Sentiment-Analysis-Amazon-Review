...
df_cell_phones = pd.read_csv('Cell_Phones_with_sentiment.csv')
df_appliances = pd.read_csv('Appliances_with_sentiment.csv')
...
# Streamlit UI
st.sidebar.title("Amazon Product ReviewIQ")
category = st.sidebar.selectbox("Select Category", ["Beauty", "Gift Cards","Cell Phones and Accessories","Appliances"])
...
def display_product_details(result):
    ...
    <div style="border: 1px solid #ddd; border-radius: 5px; padding: 10px; margin-bottom: 20px;">
        <h3 style="color: #1f77b4;">ASIN: {result['ASIN']}</h3>
        <p><strong>Positive Sentiment Score:</strong> {result['Positive Sentiment Score']}</p>
        <p><strong>Negative Sentiment Score:</strong> {result['Negative Sentiment Score']}</p>
        <p><strong>Neutral Sentiment Score:</strong> {result['Neutral Sentiment Score']}</p>
        <p><strong>Top 10 Positive Rank:</strong> {result['Top 10 Positive Rank']}</p>
        <p><strong>Top 10 Negative Rank:</strong> {result['Top 10 Negative Rank']}</p>
    </div>
...
if search_button and asin_input:
    ...
    if category == "Beauty":
        st.markdown("<h2 style='text-align: center; color: #1f77b4;'>Beauty Category Analysis</h2>", unsafe_allow_html=True)
    else:
        st.markdown("<h2 style='text-align: center; color: #1f77b4;'>Gift Cards Category Analysis</h2>", unsafe_allow_html=True)

    st.markdown("""
    <div style="display: inline-block; position: relative;">
        <h3 style="margin-bottom: 0;">Average Sentiment Scores by Rating 🛈</h3>
